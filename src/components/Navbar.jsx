...
function ThemeToggle() {
  ...
  return (
    <button
      ...
      className="group relative grid h-9 w-9 cursor-pointer place-items-center overflow-hidden rounded-xl border border-agro-border bg-agro-card text-agro-muted transition-colors duration-200 hover:border-agro-accent/50 hover:text-agro-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-agro-accent/60 sm:h-10 sm:w-10"
    >
      <span
...
export default function Navbar({ view, onChange }) {
  return (
    <header className="relative z-[1200] border-b border-agro-border bg-agro-bg/95 backdrop-blur">
      <div className="mx-auto flex h-14 w-full max-w-[1600px] items-center justify-between gap-3 px-3 sm:h-16 sm:px-5">
        {/* Logo */}
        <button
          onClick={() => onChange('map')}
          className="flex shrink-0 cursor-pointer items-center gap-2 rounded-lg outline-none focus-visible:ring-2 focus-visible:ring-agro-accent/60"
        >
            ...
            {NAV_ITEMS.map(({ id, label, short, Icon }) => {
              ...
              return (
                <button
                  ...
                  aria-pressed={active}
                  className={[
                    'flex cursor-pointer items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold transition-all duration-200 sm:px-4',
                    active
